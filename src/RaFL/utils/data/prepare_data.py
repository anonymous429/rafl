...
from .datasets import MNIST_truncated, FashionMNIST_truncated, SVHN_custom, CIFAR10_truncated, Generated, \
    FEMNIST, CelebA_custom, CIFAR100_truncated, TinyImageNet_truncated
from .. import mkdirs,get_logger

logger = get_logger()

def load_imgnet_data(datadir):


    transform = transforms.Compose([transforms.ToTensor()])
    train_ds = TinyImageNet_truncated(datadir, train=True, download=True, transform=transform)
    test_ds = TinyImageNet_truncated(datadir, train=False, download=True, transform=transform)
...
def partition_data(dataset, datadir, logdir, partition, n_parties, beta=0.4):
    ...
    if dataset == 'mnist':
        ...
        X_train, y_train, X_test, y_test = load_imgnet_data(datadir)
    elif dataset == 'fmnist':
...
def get_dataloader(dataset, datadir, train_bs, test_bs, dataidxs=None, noise_level=0, net_id=None, total=0, n_worker=32):
    if dataset in ('tinyimagenet','mnist', 'femnist', 'fmnist', 'cifar100','cifar10', 'svhn', 'generated', 'covtype', 'a9a', 'rcv1', 'SUSY'):
        if dataset == 'mnist':
